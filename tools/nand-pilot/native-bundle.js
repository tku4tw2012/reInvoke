...
assert(process.argv.length === 6,
  'usage: node native-bundle.js ARCHIVE PILOT_ARTIFACT COMPACT_BSL_ARTIFACT NEW_OUTPUT; kernel overrides are not supported');
...
assert.deepEqual(records.filter(record => !record.vendorPayloadUnchanged).map(record => record.name),
  ['rootfs', 'bsl'], 'only owned filesystems may differ');
for (const name of ['bootimgs', 'bootimgs_B'])
  assert(records.find(record => record.name === name).vendorPayloadUnchanged,
    `${name} must retain the complete vendor kernel container`);
assert.equal(records.find(record => record.name === 'app').dataType, 1);
// Only the two filesystems this builder owns may differ from the vendor
// payload; the kernel containers are asserted unchanged above.
const changedRecords = ['rootfs', 'bsl'];
