...
`);
  ...
  replace('for service_name in mic-capture provision-windowd dsp-interface \\\n' +
    ...
    '      pairing-agent bluedroid servicemanager identifiers propertyd \\\n' +
...
log "NAND pilot RC12 runtime dispatched; health and NAND origin require evidence, not this message"`);
  ...
  replace([
    ...
    '      fi',
    '      # The donor kit ships this binary; it was simply never packaged. It',
    '      # must be running before the stack starts, and it needs',
    '      # LD_LIBRARY_PATH rather than the loader --library-path form, which',
    '      # trips a getpagesize assertion.',
